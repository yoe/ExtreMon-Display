...
public class CountdownSetAction extends AbstractAction
{
	...
	public CountdownSetAction(X3Panel panel,Element element,String attribute,String format)
	{
	...
	{			
		try
		{
			...
			if(years>0)
			{
				formatted.append(years + (years==1?" year ":" years "));
			...
			if(months>0)
			{
				formatted.append(months + (months==1?" month ":" months "));
			...
			if(weeks>0)
			{
				formatted.append(weeks + (weeks==1?" week ":" weeks "));
			...
			if(hours>0)
			{
				formatted.append(hours + (hours==1?" hour ":" hours "));
			...
			if(minutes>0)
			{
				formatted.append(minutes + (minutes==1?" minute ":" minutes "));
				secondsLeft-=minutes*MINUTES;
			}
			
			if(secondsLeft>0)
			{
				formatted.append(secondsLeft + (secondsLeft==1?" second":" seconds"));
			}
			...
			final String formattedValue=String.format(getFormat(),formatted.toString());	
